...
public class AvailabilityService {
    ...
    public List<AvailableSlot> getAvailableSlots(UUID userId, LocalDate date) {
        ...
        return computeSlots(userId, date, wakeup, sleep);
    }
    ...
    public Map<LocalDate, List<AvailableSlot>> getAvailableSlotsForRange(UUID userId, LocalDate startDate, LocalDate endDate) {
        ...
        for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
            result.put(date, computeSlots(userId, date, wakeup, sleep));
        }
        return result;
    }

    private List<AvailableSlot> computeSlots(UUID userId, LocalDate date, LocalTime wakeup, LocalTime sleep) {
        ...
        List<ZoneResponse> zones = zoneService.getZonesByDate(userId, date).stream()
                .sorted(Comparator.comparing(ZoneResponse::startTime))
                .toList();

        LocalDateTime dayStart = date.atStartOfDay();
        LocalDateTime dayEnd = date.plusDays(1).atStartOfDay();
        List<Session> sessions = sessionRepository.findByUserIdAndDateRange(userId, dayStart, dayEnd);
